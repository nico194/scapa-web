...
function pictogramsReducer(state = initialState, { type, payload }) {
  switch (type) {
    ...
    case UPDATE_PICTOGRAM_SUCCESS: {
      return {
        ...state,
        pictograms: state.pictograms.map((pic) =>
          pic.id === payload.pictogram.id
            ? {
                ...pic,
                attributes: {
                  description: payload.pictogram.attributes.description,
                  image_url: payload.pictogram.attributes.image_url,
                },
                relationships: {
                  classifiable: {
                    data: {
                      id: payload.pictogram.relationships.classifiable.data.id,
                    },
                  },
                },
              }
    ...
    case DELETE_PICTOGRAM_SUCCESS: {
      return {
        ...state,
        loadingPictograms: false,
        pictograms: state.pictograms.filter(
          (pictogram) => pictogram.id !== payload.pictogram.id
        ),
      };
    }
