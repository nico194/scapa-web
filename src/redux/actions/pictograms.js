...
export const getPictograms = (
  ...
  return (dispatch) => {
    ...
    axiosConfig
      .get(
        `/admin/pictograms?per_page=${perPage}&page=${page}&by_description=${byDescription}`,
        headers
      )
      .then((response) =>
        dispatch({
...
export const addPictogram = (pictogramToAdd, { accessToken, client, uid }) => {
  return async (dispatch) => {
    ...
    const imageInBase64 = await toBase64(pictogramToAdd.attributes.image_url);
    const pictogram = {
      pictogram: {
        ...
        image: {
          data: imageInBase64,
          filename: `${pictogramToAdd.attributes.description}.png`,
        },
      },
    };
    try {
      ...
      return dispatch({
        type: ADD_PICTOGRAM_SUCCESS,
        payload: { pictogram: response.data.data },
      });
    } catch (err) {
      return dispatch({
        type: ADD_PICTOGRAM_ERROR,
        payload: { err: err.response.data.error },
      });
    }
  };
...
export const updatePictogram = (
  ...
  return async (dispatch) => {
    ...
    try {
      ...
      if (typeof pictogramToUpdate.attributes.image_url !== 'string') {
        const imageInBase64 = await toBase64(
          pictogramToUpdate.attributes.image_url
        );
        pictogram.pictogram.image = {
          data: imageInBase64,
          filename: `${pictogramToUpdate.attributes.description}.png`,
        };
      }

      const response = await axiosConfig.put(
        `/admin/pictograms/${pictogramToUpdate.id}`,
        JSON.stringify(pictogram),
        headers
      );
      return dispatch({
        type: UPDATE_PICTOGRAM_SUCCESS,
        payload: { pictogram: response.data.data },
      });
    } catch (err) {
      return dispatch({
        type: UPDATE_PICTOGRAM_ERROR,
        payload: { err: err.response.data.error },
      });
    }
  };
...
export const deletePictogram = (id, { accessToken, client, uid }) => {
  return (dispatch) => {
    ...
    axiosConfig
      .delete(`/admin/pictograms/${id}`, headers)
      .then((response) =>
        dispatch({
          type: DELETE_PICTOGRAM_SUCCESS,
          payload: { pictogram: response.data.data },
        })
      )
      .catch((err) =>
        dispatch({
          type: DELETE_PICTOGRAM_ERROR,
          payload: { err: err.response.data.error },
        })
      );
  };
...
export const filterPictogramsByCategory = (
  idCategory,
  { accessToken, client, uid },
  perPage,
  page
  ...
  return (dispatch) => {
    ...
    axiosConfig
      .get(
        `/admin/pictograms?category_id=${idCategory}&page=${page}&per_page=${perPage}`,
        headers
      )
      .then((response) => {
        dispatch({
