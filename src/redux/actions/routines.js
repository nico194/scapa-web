...
export const updateRoutine = (phraseToEdit, { accessToken, client, uid }) => {
	return async dispatch => {
		...
		try {
			const response = await axiosConfig.put(`/admin/routines/${phraseToEdit.id}`, JSON.stringify(phraseRequest), headers);
			const routine = { ...phraseToEdit, id: response.data.data.id, type: response.data.data.type }
			return dispatch({ type: UPDATE_ROUTINE_SUCCESS, payload: { routine } });
		} catch (err) {
			console.log(err)
			return dispatch({ type: UPDATE_ROUTINE_ERROR, payload: { err } })
		}
	}
}
